...
def runFiles(    rawData_dir,    outputFolder_name,    remove_words=["建设","新建","高峰","前进","公安","海湾","楼下","西边","中村"]    ):
    ...
    outputFolder_combine = f"data/{outputFolder_name}/combine"

    if not os.path.exists(f"data/{outputFolder_name}"):
        os.mkdir(f"data/{outputFolder_name}")
    os.mkdir(outputFolder_hanlp)
    os.mkdir(outputFolder_match)
    os.mkdir(outputFolder_combine)

    print(f "Files are prepared. ")
    print(f"Running rawdata files from {rawData_dir} to {outputFolder_name}...")
    print(f"Your final files will be saved in the following folders: {outputFolder_combine}")

...
def match_run_oneFile(my_filepath, def match_run_oneFile(my_filepath):
    ...
    for text in texts_list:
        tmp = []
        for patch in list(def match_run_oneFile(my_filepath):
    texts = read_docx(my_filepath)
    texts_list = texts.split("\n")
    ...
    for text in texts_list:
        ...
        ner_list.append(tmp)
    my_filename = my_filepath.split("/")[-1].split(".")[0]
    return ner_list, my_filename["location_name"].values):
            if patch.strip() in text:
                tmp.append([patch.strip(),"ns",0,0])
            # else:
            #     tmp.append([])
        ...
        #* if alias in text, then add the location_name to the list
        for _, row in def match_run_oneFile(my_filepath):
    texts = read_docx(my_filepath)
    texts_list = texts.split("\n")
    ...
    for text in texts_list:
        ...
        ner_list.append(tmp)
    my_filename = my_filepath.split("/")[-1].split(".")[0]
    return ner_list, my_filename.dropna().iterrows():
            for i in row["alias"].split(" "):
                if i.strip() in text:
                    tmp.append(row["location_name"],"ns",0,0)
                    break
...
def match_run_oneFolder(input_folder,output_folder):
    fileList=os.listdir(input_folder)
    for i in tqdm(fileList):
        my_filepath = f"{input_folder}/{i}"
        ner_list, filename = match_run_oneFile(my_filepath)
        # display(ner_list)
        with open(f"{output_folder}/{filename}.txt","w") as f:
            f.write("[\n")
            for item in ner_list:

...
def hanlp_oneFile(my_filepath, HanLP):
...
def hanlp_runFiles(my_fileList,output_folder):
    print("Hanlp is running...")
    # fileList=os.listdir(input_folder)
    myHanLP = HanLPClient('https://www.hanlp.com/api', auth="NTc3MkBiYnMuaGFubHAuY29tOnV6R0xMS05pblB3c29CZE4=", language='zh')

    for i in tqdm(my_fileList):
        # my_filepath = f"{input_folder}/{i}"
        # ner_list, filename = hanlp_oneFile(my_filepath,myHanLP)
        ner_list, filename = hanlp_oneFile(i,myHanLP)
...
def match_oneFile(my_filepath, matchFrame):
...
def match_runFiles(my_fileList,output_folder,matchFrame):
    prnt("Match is running...")
    for i in tqdm(my_fileList):
        ner_list, filename = match_oneFile(i,matchFrame)
        with open(f"{output_folder}/{filename}.txt","w") as f:
            f.write("[\n")
            for item in ner_list:
                f.write("%s\n" % item)

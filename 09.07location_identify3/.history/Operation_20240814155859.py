...
def match_run_oneFile(my_filepath, match_df):
    ...
    for text in texts_list:
        tmp = []
        for patch in list(match_df["location_name"].values):
        ...
        for _, row in match_df.dropna().iterrows():
...
def match_run_oneFolder(input_folder,output_folder,match_df
    fileList=os.listdir(input_folder)
    for i in tqdm(fileList):
        my_filepath = f"{input_folder}/{i}"
        ner_list, filename = match_run_oneFile(my_filepath,match_df)
        # display(ner_list)
        with open(f"{output_folder}/{filename}.txt","w") as f:
            f.write("[\n")
            for item in ner_list:
